...
class XiaoMusicDevice:
    ...
    async def download(self, search_key, name):
        ...
        # cow: inject browser UA+referer for bilibili search, mitigate HTTP 412
        if not any(a == "--user-agent" for a in sbp_args):
            sbp_args += (
                "--user-agent",
        ...
        # cow: cap audio stream bitrate to <=320 kbps (pick a lower one when available)
        sbp_args += ("-f", "ba[abr<=320]/ba")
        ...
        chmodfile(file_path)
        # cow: 下载已完成，此时文件才完整。强制重算这一首的时长，
        # 覆盖下载中途目录扫描写进去的「半截文件」错值。
        try:
            await self.xiaomusic.music_library.regen_duration_for([name])
        except Exception as e:
            self.log.warning(f"cow: 下载完成后重算时长失败 {name}: {e}")
    ...
    async def play_music_list(self, list_name, music_name):
        ...
        # 总列表中指定某首歌，按播放单曲处理；具体歌单中点某首歌，仍按歌单处理。
        total_lists = ("全部", "所有歌曲", "所有电台")
        if list_name in total_lists and music_name:
            await self._auto_switch_play_type(PLAY_TYPE_ONE, f"播放单曲【{music_name}】")
        elif list_name not in total_lists:
            await self._auto_switch_play_type(PLAY_TYPE_ALL, f"播放歌单【{list_name}】")

        # cow: 修复「播放歌单X」误播/误下载。
        #
        # 原逻辑直接 _play(music_name)，而 _play 允许下载(_play_internal
        # allow_download=True)。当 music_name 为空时 _play("") 会退化成
        # “取当前正在播的歌”，而这首歌往往不属于刚切过来的歌单，匹配不到
        # 本地文件时就触发网络搜索下载（表现为说完“播放歌单蔡琴”却开始
        # 下载那首不相干的歌）。
        #
        # 修法：
        #   1) music_name 由调用方明确传入（API / 在线歌单指定某首）→ 行为不变；
        #   2) music_name 为空（语音“播放歌单X”从头播）→
        #      a. 记忆位置 playlist2music 里的歌名必须属于该歌单，否则丢弃
        #         （它可能来自上一个歌单，是残留值）；
        #      b. 丢弃后回退到歌单第一首；
        #      c. 全程 allow_download=False —— 播放歌单这条路永不下载。
        explicit = bool(music_name)

        if not explicit:
            remembered = self.device.playlist2music.get(list_name, "")
            if remembered and remembered not in self._play_list:
                self.log.info(
                    f"cow: 歌单【{list_name}】的记忆歌曲【{remembered}】"
                    f"不属于该歌单，忽略"
                )
                remembered = ""
            music_name = remembered

        if not explicit and not music_name:
            if not self._play_list:
                self.log.info(f"cow: 歌单【{list_name}】为空，无法播放")
                return
            music_name = self._play_list[0]
            self.log.info(f"cow: 歌单【{list_name}】从头播放：{music_name}")
        ...
        if explicit:
            ...
            await self._play_internal(
                name=music_name, search_key="", allow_download=False
            )
